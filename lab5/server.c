...
int main(int argc, char *argv[]) {
    ...
    load_credentials(); // load client credentials from pseudo db
...
void handle_message(int sockfd, message *msg) {
    switch (msg->type) {
        ...
        case DIRECT_MESSAGE:
            ...
            break;
        case REGISTER:
            handle_register_client(sockfd, msg);
            break;
        default:
...
void handle_login(int sockfd, message *msg) {
    ...
    bool client_found = check_user_existence(msg->source);
    bool password_valid = validate_credential(msg->source, password);

    printf("client found: %d, pass valid: %d\n", client_found, password_valid);
    message response = {0};
    response.type = LO_ACK;

    if (!client_found || !password_valid) {
        char* reason;
        if (!client_found) {
            reason = "client ID does not exist";
...
void handle_direct_message(int sockfd, message *msg) {
    char recipient_id[MAX_NAME], message_content[BUFFER_SIZE];
    decode_direct_message(msg->data, recipient_id, message_content);

    message failure_msg = {0};
    failure_msg.type = DM_NAK;

    if (!check_user_existence(recipient_id)) {
        failure_msg.size = snprintf(failure_msg.data, MAX_DATA, "recipient id '%s' does not exist", recipient_id);
        send_message(sockfd, &failure_msg);
        return;
    }

    client *c = get_client_by_id(recipient_id);
    if (c == NULL) {
        failure_msg.size = 
        send_message(sockfd, &failure_msg);
        return;
    }
    send_message(c->sockfd, msg);
}

void handle_register_client(int sockfd, message *msg) {
    char client_id[MAX_NAME], password[MAX_NAME];
    decode_register_message(msg->data, client_id, password);
    message response = {0};

    if (check_user_existence(client_id)) {
        response.type = REGISTER_NACK;
        response.size = snprintf(response.data, MAX_DATA, "client id '%s' already exists", client_id);
        send_message(sockfd, &response);
        return;
    }
    add_credential(client_id, password);
    response.type = REGISTER_ACK;
    response.size = snprintf(response.data, MAX_DATA, "sucessfully registered user '%s'", client_id);
    send_message(sockfd, &response);
}
...
void add_client(int sockfd, char* client_id, char* password, char** failure_reason) {
    *failure_reason = NULL;
    ...
    // check if client id already connected
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (connected_clients[i] != NULL && strcmp(connected_clients[i]->client_id, client_id) == 0) {
            *failure_reason = "client ID already connected";
            pthread_mutex_unlock(&mut_clients);
            return;
        }
    }
